<?php
    ...
    //query empresas
    $query= mysqli_query($conn, "SELECT id, nombre from empresas WHERE estado='Activo'");
    // echo $query;
    $i=0;
    $empresas = "";
    while($row_2 = mysqli_fetch_array($query, MYSQLI_ASSOC))
    {
     $empresas=$empresas.'<option value="'. $row_2["id"] . '">' . $row_2["nombre"].  "</option>";
        $i++;
    }
?>
                ...
                <div class="row justify-content-center">
                  ...
                  <div class="col-sm-3">
                    <div class="form-group select-wizard">
                      <label>Empresa</label>
                      <select required class="selectpicker" id="empresa" name="empresa" data-size="7" data-style="select-with-transition" title="Seleccionar">
...
<script>
  ...
  var obtener_data_editar = function(tbody, table) {
    $(tbody).on("click", "button.editar", function() {
      ...
      empresa = $('#empresa option[value="' + data.empresa + '"]').prop("selected", "selected").change(),
