  ...
  if ($("#session").val() == "Bodeguero"){
    $("#link_total_ventas").hide();
  }

  if ($("#session").val() == "Contador"){
    $("#link_productos").hide();
  }
  ...
  var producto_mas_vendido = function(){
    $.ajax({
      ...
      success: function(data){
        ...
        if (json_info['data']=="" || json_info['data']==null){
          var fecha_actual = new Date().toLocaleString().replace(',','');
          $("#contador_exito").html('No hay registros');
          $("#fecha_exito").html('- '+fecha_actual);
        }else{
  ...
  var producto_menos_vendido = function(){
    $.ajax({
      ...
      success: function(data){
        ...
        if (json_info['data']=="" || json_info['data']==null){
          var fecha_actual = new Date().toLocaleString().replace(',','');
          $("#contador_menos").html('No hay registros');
          $("#fecha_menos").html('- '+fecha_actual);
        }else{
