                            ...
                            if(empty($filasNoNumericasCantidad)){
                                ...
                                if(empty($DatosExistentes)){
                                    ...
                                    for ($row = 2; $row <= $highestRow; $row++){
                                        ...
                                        $fecha_venc = date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($sheet->getCell("G" . $row)->getValue()));

                                        $query = "INSERT INTO id18070131_sivenpro.productos (nombre_producto, cod_producto, cantidad, precio, marca, cont_neto, fecha_venc) VALUES";
                                        $query .= "('$nombre', \"$codigo\", \"$cantidad\", \"$precio\", \"$marca\", \"$cont_neto\", \"$fecha_venc\")";
                                        $conn->query($query);
...
function verificaCodBD($sheet,$highestRow,$conn){
    $tabla = 'id18070131_sivenpro.productos';
    $campo = 'cod_producto';
    $arregloCod = array();
