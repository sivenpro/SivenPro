...
include("generaPdf_EPP.php");

$id_orden = $_POST["id_orden"];

/* $conn es la conexión ! */

$query = "SELECT a.*, b.nombre_completo as encargado FROM sistema_interno.registro_epp a
          INNER JOIN sistema_interno.usuarios b ON b.id = a.usuario
          WHERE a.id = '$id_orden'";
// echo $query;
...
if (!$resultado) {
    ...
    $sql_materiales = "SELECT * FROM sistema_interno.materiales_epp WHERE id_registro = '$id_orden'";
    $sql_firma = "SELECT nombre_completo, c.nombre as nombre_perfil, url_firma, DATE_FORMAT(b.fecha_modif,'%d-%m-%Y') as fecha_modif_
                  FROM sistema_interno.usuarios a
                  INNER JOIN sistema_interno.registro_epp b ON a.nombre = b.nombre_usuario
            ...
            <table style="border-collapse: collapse; margin-bottom: 2%; width: 100%;" border-style: none;" border="0">
              <tbody>
                  <tr style="height: 23px;">
                    <td style="width: 33.3%; text-align: center; height: 21px;"><strong>Usuario Encargado</strong></td>
                  ...
                  <tr style="height: 21px;">
                    <td style="width: 33.3%; text-align: center; height: 21px;">' . $data_final->encargado . '</td>
    ...
    if ($data_final->lista_materiales != null && count($data_final->lista_materiales) > 0) {
        $html_lista_materiales = '<h4>Materiales asignados</h4>
            <table style="border-collapse: collapse; margin-bottom: 3%; width: 100%; border-color: #d3d3d3; border-style: dotted; " border="1">
              <tbody>
                <tr style="height: 21px;">
                  <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; "><strong>C&oacute;digo Material</strong></td>
                  <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; "><strong>Descripci&oacute;n</strong></td>
                  <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; "><strong>Cantidad</strong></td>
                </tr>';
        ...
        foreach ($data_final->lista_materiales as $key) {
            ...
            $total_clp_materiales += $key->valor;
                $html_lista_materiales .=
                '<tr style="height: 23px;">
                <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; ">' . $key->id_material . '</td>
                <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; ">' . $key->descripcion_material . '</td>
                <td style="width: 33.3%; text-align: center; height: 21px; border-style: dotted; ">' . $key->cantidad . '</td>
                </tr>';
        }
