...
error_reporting(E_ALL);  */
 ...
 mysql_select_db('gassur_otros');
 mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8',
     character_set_database = 'utf8', character_set_server = 'utf8'"); 

setlocale(LC_ALL, "es_ES", 'Spanish_Spain', 'Spanish');
  
$id_usuario= $_SESSION["usuario_movil_id"];
	
//require_once __DIR__ .'../../../gassur/mail/pantilla/vendor/autoload.php';	
...
$id=$_POST['id'];  
$query_usuario = "SELECT nombre_completo, posee_firma FROM gassur_otros.usuarios_nuevo where id = $id_usuario";
$resultado = mysqli_query($conn, $query_usuario);
$row_usuario = mysqli_fetch_assoc($resultado);
$nombre_completo = $row_usuario['nombre_completo'];
$posee_firma = $row_usuario['posee_firma'];

$nombre_firma = strtoupper($nombre_completo);
switch ($posee_firma) {
	case 'Sí':
		$ruta_firma = "../../modulos/otros/usuarios_nuevo/firmas/".$id_usuario."_firma.jpg";
		break;
	case 'No':
		$ruta_firma = "../../modulos/otros/usuarios_nuevo/firmas/sinfirma.png";
		break;
	
}


$query_llae="SELECT * FROM gassur_otros.llae_nuevo 
                WHERE aviso = '$id'";
        $resultado = mysqli_query($conn, $query_llae);
		$row_llae = mysqli_fetch_assoc($resultado);
        
		$nombre = $row_llae['empresa_persona']; 
		$empresa = $row_llae['empresa_persona']; 
		$fono = $row_llae['telefono_contacto']; 
		$localidad = $row_llae['poblacion']; 
		$n_llae_sap = $row_llae['aviso'];
		$cargo = "a"; 
		$correlativo_gassur = $row_llae['aviso'];; 
		$sector = $row_llae['calle']; 
		$tipo = $row_llae['existencia_red']; 
		if ($tipo=="No") {
			$tipo="Sin";
		}else{
		$query_planos="SELECT * FROM gassur_otros.galeria_planos 
                	   WHERE codigo = '$id' 
                	   AND modulo='llae'
                	   AND estado='Activo'";
        $resultado = mysqli_query($conn, $query_planos);
		while($row_planos = mysqli_fetch_assoc($resultado))
        {
		//$cuenta_planos ++;
		$nombre_corto = substr($row_planos['url_archivo'], 0, -4);
		$array_plano[] = $nombre_corto;
		//$planos .= $nombre_corto.', ';
		}
		$array_plano = array_unique($array_plano);
		$array_plano = array_values($array_plano);
		
		for ($i = 0; $i < count($array_plano); $i++) {
			$cuenta_planos++;			
			$planos .= $array_plano[$i] . ', ';
		}
		...
		if ($cuenta_planos == 1) {
			$planos = 'Plano Adjunto N° '. $planos;
		}else{
			$planos = 'Planos Adjuntos N° ' . $planos;
		}
		}
...
/* $nombre= "a"; //$_POST['nombre']; // nombre de la persona que lo solicito
$empresa= "a"; //$_POST['empresa']; // nombre de la empresa que lo solicito	
$fono= "a"; //$_POST['fono']; // telefono
$localidad= "a"; //$_POST['localidad']; // hay localidades donde no existe el servicio de gassur , por consecuencia la respuesta de la carta es 'Sin'
$n_plano= "a"; //$_POST['n_plano'];
$n_llae_sap= "a"; //$_POST['n_llae_sap']; // es el nuevo identificador del LLAE 
$cargo= "a"; //$_POST['cargo']; // era el cargo de la persona que solicitaba el llae, no se si en el nuevo formulario existe este campo. 
$correlativo_gassur= "a"; //$_POST['correlativo_gassur'];
$sector= "a"; //$_POST['sector'];
$tipo= "a"; //$_POST['tipo']; // se refiera a que tipo de carta debe generar, es una variable interna. ejemplo = linea 68  */
...
LLAE $n_llae_sap/$anio</FONT></FONT></P>
...
<P CLASS="western" ALIGN=JUSTIFY STYLE="margin-top: 0.19in; margin-bottom: 0.19in"><FONT FACE="Arial Narrow, serif"><FONT SIZE=2 STYLE="font-size: 11pt">En
...
<img style="margin-left:180px;margin-bottom:-30px" src="$ruta_firma" >
</div>

<P CLASS="western" ALIGN=CENTER style="font-size:14px">

<span  STYLE="margin-bottom: -15px"><FONT FACE="Arial Narrow, serif"><FONT SIZE=14px><B> $nombre_firma </B>
<br>
Calidad y Seguridad Redes</FONT></FONT></span>
...
$localidad, $fechacarta
N° LLAE $correlativo_gassur/$anio
</P>
...
Gas Sur informa que posee redes de distribución de gas Natural como se indica en $planos
<br>
<br>
...
<span  STYLE="margin-bottom: -15px"><FONT FACE="Arial Narrow, serif"><FONT SIZE=12px><B>$nombre_firma</B>
<br>
Calidad y Seguridad Redes</FONT></FONT></span>
</P>
<P CLASS="western2" STYLE="font-size: 11px">
...
mkdir("/var/www/html/test/osm2/galeria/uploads/llae/$id");

// First save it to the server somewhere 
$mpdf->Output("/var/www/html/test/osm2/galeria/uploads/llae/$id/".$filename.".pdf", "F"); // Make sure the path is valid and writing permissions are set correctly to prevent writing errors.
...
$query2 = "update gassur_otros.llae_nuevo set posee_carta='Sí' where aviso=".$id;
